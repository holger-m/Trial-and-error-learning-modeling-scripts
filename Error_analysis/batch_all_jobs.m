...
load(['C:\data_path\Lost_probabilities\Lost_probs_',model_str,'.mat']);


if model_no == 0
    
    Errors_types_3D = NaN(9,15,85);

else
    
    Errors_types_3D = NaN(9,1000,15,85);
...
for subj = 1:85
    ...
    for block = 6:20        
        ...
        if model_no == 0
            ...
            lost_prob_vec = Lost_probs(1:block_length, block - 5, subj);
            
            resp_prob_vec = Resp_probs(1:block_length, block - 5, subj);
            
            errors_types_1_to_9 = errortypes_run_block(current_stim_vec, current_resp_actu_vec, current_resp_corr_vec, lost_prob_vec, resp_prob_vec);

            Errors_types_3D(:, block - 5, subj) = errors_types_1_to_9;
            ...
            for sim_no = 1:1000
                ...
                lost_prob_vec = Lost_probs(1:block_length, sim_no, block - 5, subj);
                
                resp_prob_vec = Resp_probs(1:block_length, sim_no, block - 5, subj);
                
                errors_types_1_to_9 = errortypes_run_block(current_stim_vec, current_resp_actu_vec, current_resp_corr_vec, lost_prob_vec, resp_prob_vec);
                
                Errors_types_3D(:, sim_no, block - 5, subj) = errors_types_1_to_9;
...
save_str = ['Error_types_1_to_9_',model_str];

save(save_str,'Errors_types_3D');
